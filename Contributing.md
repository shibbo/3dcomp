...
Feel free to join the [3D Mario Reverse Engineering](https://discord.gg/QnZ4cKkZm3) Discord server if you have any questions.

## Language

This project is mostly written in C++17.

Unlike the vast majority of games that are being decompiled in 2021, *Super Mario 3D World + Bowser's Fury* is a large modern game that is written in C++. While C and C++ have a similar syntax, C++ is somewhat more complex than C. To avoid getting lost in C++ code, please familiarize yourself with the following basic language concepts *before* decompiling:
...
3dcomp is mostly set up like a normal C++ project using standard build tools and compilers like Clang, CMake, Ninja, etc. so autocomplete and "IntelliSense" style features should work almost out-of-the-box.
...
5. **Add the function name to the list of decompiled functions.**
    * To do so, open `data/bf_functions.csv`, search for the name or the address of function you have decompiled, and add the function name to the last column.
...
* Use `#include "..."` when including `al` and `rs` (3DW) header files.
