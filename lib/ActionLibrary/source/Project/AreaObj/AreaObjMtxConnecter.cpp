...
namespace al {
    ...
    bool AreaObjMtxConnecter::trySetParentMtx(const sead::Matrix34f* pMtx,
                                              const PlacementInfo& rInfo) {
        if (!al::isEqualPlacementID(_40, rInfo)) {
            return false;
        }
    ...
    bool AreaObjMtxConnecterHolder::tryAddArea(AreaObj* pArea, const PlacementInfo& rInfo) {
        s32 linkNum = al::calcLinkChildNum(rInfo, "NoDelete_FollowMtxTarget");

        if (linkNum != 0) {
            PlacementInfo info;
            al::getLinksInfo(&info, rInfo, "NoDelete_FollowMtxTarget");
    ...
    void AreaObjMtxConnecterHolder::registerParentMtx(const sead::Matrix34f* pMtx,
        ...
        for (s32 i = 0; i < mNumConnecters; i++) {
            auto connecter = mConnecters[i];

            if (al::isEqualPlacementID(connecter->_40, rInfo)) {
                auto mtxCon = new MtxConnector();
